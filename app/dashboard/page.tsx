...
function StatCardSkeleton() {
  return (
    ...
    >
      ...
      />
      <div
        style={{ height: "0.68rem", width: "50%", background: "#f0f0f0", borderRadius: "4px" }}
      />
    </div>
  );
}
...
export default function Dashboard() {
  ...
  const activeWeddingId = session.status === "authenticated" ? session.activeWeddingId : null;
  ...
  useEffect(() => {
    ...
    async function fetchStats() {
      ...
      try {
        ...
        const [statsJson, taskCtxJson] = await Promise.all([statsRes.json(), taskCtxRes.json()]);
        ...
        if (taskCtxRes.ok && taskCtxJson.success) {
          ...
          const daysUntilWedding = statsData.wedding.event_date
            ? Math.max(
                0,
                Math.ceil(
                  (new Date(statsData.wedding.event_date).getTime() - Date.now()) /
                    (1000 * 60 * 60 * 24)
                )
              )
            : 365;
          ...
          const taskCtx: TaskEngineContext = {
            ...
            guestsUnassigned: Math.max(
              0,
              statsData.stats.guests_total - statsData.stats.seated_guests_total
            ),
            ...
            >
              ...
              {stats.stats.guests_total > 0 && (
                ...
                >
                  <div
                    style={{
                      width: `${stats.stats.response_rate}%`,
                      height: "100%",
                      background: "#48BB78",
                      borderRadius: "2px",
                    }}
