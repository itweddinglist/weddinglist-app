...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const [vendorsResult, paymentsDueResult] = await Promise.all([
      supabaseServer.from("vendors").select("category, status").eq("wedding_id", weddingId),
    ...
    if (vendorsResult.error) {
      return internalErrorResponse(
        vendorsResult.error,
        "GET /api/dashboard/task-context — vendors"
      );
    }
    if (paymentsDueResult.error) {
      return internalErrorResponse(
        paymentsDueResult.error,
        "GET /api/dashboard/task-context — payments_due"
      );
    }

    const vendors = vendorsResult.data ?? [];

    const has_location = vendors.some((v) => v.category === "location" && v.status === "booked");
    const has_catering = vendors.some((v) => v.category === "catering" && v.status === "booked");
