  ...
  return (
    <aside className="sc-sidebar">
      <div className="sb-search-wrap">
        <input className="sb-search" placeholder="Caută un invitat..." value={searchQuery} onChange={e=>setSearchQuery(e.target.value)}/>
      ...
      {guestMeta.groups.length > 0 && (
        <div style={{padding:'0 1rem 0.5rem'}}>
          ...
          {guestMeta.groups.map(g => (
            <div key={g.name} style={{display:'flex',alignItems:'center',gap:'0.5rem',padding:'0.2rem 0',cursor:'pointer'}}
      ...
      <div className="sb-divider"/>
      <div className="sb-section-label sb-label-pad">Neatribuiți <span className="sb-badge">{filteredUnassigned.length}</span></div>
      <div className="sb-guests">
        ...
        {allSeated && (
          <div style={{textAlign:'center',padding:'2rem 1rem'}}>
            <div style={{fontSize:'1.5rem',marginBottom:'0.5rem'}}>✨</div>
            <div style={{fontSize:'0.72rem',color:'#48BB78',fontWeight:500,marginBottom:'0.3rem'}}>Toți invitații au un loc!</div>
            <div style={{fontSize:'0.62rem',color:'#6E7490',lineHeight:1.5}}>Planul de mese este complet.</div>
          </div>
        )}
        {noResults && (
          <div style={{textAlign:'center',padding:'2rem 1rem'}}>
        ...
        {filteredUnassigned.map(g=>{
          ...
          return (
            <div key={g.id} className="sb-guest-row" draggable="true"
              onDragStart={(e)=>{setIsDraggingGuest(true);setHoveredGuest(null);e.dataTransfer.setData('guestId',String(g.id));}}
              onDragEnd={()=>setIsDraggingGuest(false)}
              onClick={()=>locateGuest(g.id)}
            >
              <div className="sb-avatar" style={{background:`${gc}22`,border:`1.5px solid ${gc}`,color:gc}}
                onMouseEnter={(e)=>{if(!isDraggingGuest){const r=e.currentTarget.getBoundingClientRect();setHoveredGuest({guest:g,x:r.right+8,y:r.top-4});}}}
                onMouseLeave={()=>setHoveredGuest(null)}
              >{g.prenume[0]+g.nume[0]}</div>
      ...
      {searchQuery && guests.filter(g => g.tableId && `${g.prenume} ${g.nume} ${g.grup}`.toLowerCase().includes(searchQuery.toLowerCase())).length > 0 && (
        <>
          <div className="sb-section-label sb-label-pad" style={{marginTop:'0.5rem'}}>
            Așezați <span className="sb-badge">{guests.filter(g => g.tableId && `${g.prenume} ${g.nume} ${g.grup}`.toLowerCase().includes(searchQuery.toLowerCase())).length}</span>
          </div>
          <div className="sb-guests" style={{maxHeight:'120px'}}>
            {guests.filter(g => g.tableId && `${g.prenume} ${g.nume} ${g.grup}`.toLowerCase().includes(searchQuery.toLowerCase())).map(g => {
              const gc = getGroupColor(g.grup);
              const table = tables.find(t => t.id === g.tableId);
              return (
                <div key={g.id} className="sb-guest-row" style={{cursor:'pointer'}}
                  onClick={() => locateGuest(g.id)}
                >
                  <div className="sb-avatar" style={{background:`${gc}22`,border:`1.5px solid ${gc}`,color:gc}}>{g.prenume[0]+g.nume[0]}</div>
                  <span className="sb-guest-name">{g.prenume} {g.nume}</span>
