...
function GuestSidebar({
  ...
  const allSeated =
    guests.length > 0 && filteredUnassigned.length === 0 && !searchQuery && !activeGroupId;
  ...
  const renderUnassignedRow = useCallback(
    (g: SeatingGuest) => {
  ...
  return (
    <aside className="sc-sidebar">
      ...
      {/* NEATRIBUIȚI */}
      <div
        className={`sb-section-unassigned${searchQuery ? "has-search" : ""}`}
        ref={unassignedRef}
      >
