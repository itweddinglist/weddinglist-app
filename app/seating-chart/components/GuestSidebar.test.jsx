...
const defaultProps = {
  guests: [
    { id: 1, prenume: 'Maria', nume: 'Popescu', grup: 'Familie Mireasă', status: 'confirmat', tableId: 1 },
    { id: 2, prenume: 'Ion', nume: 'Ionescu', grup: 'Familie Mire', status: 'asteptare', tableId: null },
  ],
  filteredUnassigned: [
    { id: 2, prenume: 'Ion', nume: 'Ionescu', grup: 'Familie Mire', status: 'asteptare', tableId: null },
  ],
  ...
  guestMeta: { groups: [{ name: 'Familie Mireasă', count: 1 }, { name: 'Familie Mire', count: 1 }] },
...
describe('GuestSidebar', () => {
  ...
  it('search input onChange → setSearchQuery apelat', () => {
    render(<GuestSidebar {...defaultProps} />);
    fireEvent.change(screen.getByPlaceholderText('Caută un invitat...'), { target: { value: 'maria' } });
