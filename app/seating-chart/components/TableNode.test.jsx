...
const renderInSvg = (props) => {
  return render(React.createElement("svg", null, React.createElement(TableNode, props)));
};
...
describe("TableNode — isSelected vizual", () => {
  it("selectedTableId === t.id → stroke #9F7AEA pe circle", () => {
    ...
    const { container } = renderInSvg(props);
    const circles = container.querySelectorAll("circle");
    const hasSelectedColor = Array.from(circles).some(
      (c) => c.getAttribute("stroke") === "#B794F4"
    );
