...
export function getSeatPositions(t: Pick<SeatingTable, "type" | "seats" | "isRing">): Point[] {
  ...
  if (t.type === "round") {
    for (let i = 0; i < t.seats; i++) {
      ...
      seats.push({
        x: (d.cx as number) + (d.seatR as number) * Math.cos(a),
        y: (d.cy as number) + (d.seatR as number) * Math.sin(a),
      });
    }
    ...
    for (let i = 0; i < counts[1]; i++)
      seats.push({ x: d.w - pad + 8, y: pad + stepR * i + stepR / 2 });
    // Jos
    const stepB = s / counts[2];
    for (let i = counts[2] - 1; i >= 0; i--)
      seats.push({ x: pad + stepB * i + stepB / 2, y: d.h - pad + 8 });
    // Stânga
    const stepL = s / counts[3];
    for (let i = counts[3] - 1; i >= 0; i--)
      seats.push({ x: pad - 8, y: pad + stepL * i + stepL / 2 });
    return seats;
  } else if (t.type === "prezidiu") {
    // tw și th sunt garantate pentru type==='prezidiu'
    const { tw } = d as { tw: number; th: number },
      step = tw / t.seats;
    for (let i = 0; i < t.seats; i++) {
    ...
    const { tw, th } = d as { tw: number; th: number },
...
export function generateCateringText(
  tables: SeatingTable[],
  ...
  real.forEach((t) => {
    ...
    const mc = tg.reduce<Record<string, number>>((a, g) => {
  ...
  const total = guests.reduce<Record<string, number>>((a, g) => {
