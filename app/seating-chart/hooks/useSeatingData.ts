...
export function useSeatingData(
  ...
  }: UseSeatingDataOptions = {}
) {
  // ── STATE ──
  const [guests, setGuests] = useState<SeatingGuestWithEvents[]>(() =>
    (initialGuests ?? INITIAL_GUESTS).map((g) => ({ ...g }))
  );
  ...
  const onSaveStatusChangeRef = useRef<((status: SaveStatus) => void) | null | undefined>(
    onSaveStatusChange
  );
  useEffect(() => {
  ...
  const onSeatingStateChangedRef = useRef<
    ((data: SeatingStateChangedData) => void) | null | undefined
  >(onSeatingStateChanged);
  useEffect(() => {
    onSeatingStateChangedRef.current = onSeatingStateChanged;
  }, [onSeatingStateChanged]);
  ...
  useEffect(() => {
    ...
    const assignmentsChanged =
      JSON.stringify(prev.assignments) !== JSON.stringify(snapshot.assignments);
    ...
    const reason: ChangeReason =
      assignmentsChanged && tablesChanged ? "both" : assignmentsChanged ? "assignments" : "layout";

    onSeatingStateChangedRef.current?.({
      reason,
  ...
  const guestById = useMemo(
    () => new Map<number, SeatingGuestWithEvents>(guests.map((g) => [g.id, g])),
    [guests]
  );
  const tableById = useMemo(
    () => new Map<number, SeatingTable>(tables.map((t) => [t.id, t])),
    [tables]
  );
  ...
  const assignGuest = useCallback(
    (gId: number | string, tableId: number): SeatingActionResult => {
      ...
      saveAction();
      setGuests((prev) =>
        prev.map((g) => (g.id === parseInt(String(gId)) ? { ...g, tableId } : g))
      );
  ...
  const unassignGuest = useCallback(
    (guestId: number | null): SeatingActionResult => {
  ...
  const magicFill = useCallback((): SeatingActionResult => {
    ...
    if (!anyUnassigned) {
      return {
        ...
        effects: [
          {
            type: "SHOW_TOAST",
            payload: { message: "Toti invitatii au deja un loc!", toastType: "yellow" },
          },
        ],
      };
    }
    ...
    const effects: SeatingEffect[] = [
      {
        type: "SHOW_TOAST",
        payload: { message: `✨ ${assignmentsCount} invitați așezați automat`, toastType: "green" },
      },
    ];
    ...
    if (prezidiuSkipped > 0) {
      effects.push({
        type: "SHOW_TOAST",
        payload: {
          message: `ℹ ${prezidiuSkipped} invitati din grupul "Prezidiu" au fost exclusi`,
          toastType: "rose",
        },
      });
    }
    if (skippedGuests.length > 0) {
      effects.push({
        type: "SHOW_TOAST",
        payload: {
          message: `⚠ ${skippedGuests.length} invitati fara loc disponibil`,
          toastType: "yellow",
        },
      });
    }
    if (skippedGroups.length === 1) {
      effects.push({
        type: "SHOW_TOAST",
        payload: { message: `⚠ ${skippedGroups[0].reason}`, toastType: "yellow" },
      });
    } else if (skippedGroups.length > 1) {
      effects.push({
        type: "SHOW_TOAST",
        payload: {
          message: `⚠ ${skippedGroups.length} grupuri nu au putut fi plasate impreuna`,
          toastType: "yellow",
        },
      });
    }
    if (limitReached) {
      effects.push({
        type: "SHOW_TOAST",
        payload: {
          message: "⚠ Solutie partiala — limita atinsa, se foloseste cel mai bun rezultat gasit",
          toastType: "yellow",
        },
      });
    }
  ...
  const createTable = useCallback(
    (modal: CreateTableModal | null): SeatingActionResult => {
      ...
      return {
        ...
        effects: [
          ...
          {
            type: "SHOW_TOAST",
            payload: { message: `✓ "${modal.name || "Bar"}" creat`, toastType: "green" },
          },
          { type: "CLOSE_MODAL" },
        ],
      };
  ...
  const deleteTable = useCallback(
    (tableId: number, _tables?: SeatingTable[]): SeatingActionResultWithConfirm => {
  ...
  const rotateTable = useCallback(
    (tableId: number, deg: number): SeatingActionResult => {
      saveAction();
      setTables((prev) =>
        prev.map((t) =>
          t.id === tableId ? { ...t, rotation: ((((t.rotation || 0) + deg) % 360) + 360) % 360 } : t
        )
      );
      return { ok: true, effects: [] };
    },
  ...
  const saveEdit = useCallback(
    (editName: string, editSeats: number, editPanelTableId: number): SeatingActionResult => {
  ...
  const resetPlan = useCallback(
    (
      dispatchCam?: ((action: { type: string; [key: string]: unknown }) => void) | null
    ): SeatingActionResult => {
      saveAction();
      ...
      return {
        ok: true,
        effects: [{ type: "SHOW_TOAST", payload: { message: "Plan resetat", toastType: "red" } }],
      };
    },
  ...
  const setTablesExternal = useCallback(
  ...
  // ── FILTERED UNASSIGNED ──
  const filteredUnassigned = useCallback(
    (searchQuery?: string | null): SeatingGuestWithEvents[] => {
  ...
  const revertToSnapshot = useCallback(
    ({ tables: snapTables, guests: snapGuests }: SeatingSnapshot) => {
