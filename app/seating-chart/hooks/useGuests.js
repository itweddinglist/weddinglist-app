...
import { PLAN_CX, PLAN_CY, PLAN_W, PLAN_H, GRID, INITIAL_GUESTS, buildTemplate, getTableDims, getGroupColor } from '../utils/geometry.js';
...
export function useGuests(cam) {
  ...
  const filteredUnassigned = useMemo(() =>
    searchQuery
      ? unassigned.filter(g => `${g.prenume} ${g.nume} ${g.grup}`.toLowerCase().includes(searchQuery.toLowerCase()))
      : unassigned,
    [unassigned, searchQuery]
  );
  const progress  = guests.length > 0 ? (assignedCount / guests.length * 100) : 0;
  const menuStats = useMemo(() => guests.reduce((acc, g) => { acc[g.meniu] = (acc[g.meniu] || 0) + 1; return acc; }, {}), [guests]);
...
const groupColorMap = useMemo(() => {
  ...
  const magicFill = useCallback(() => {
    const anyUnassigned = guestsRef.current.some(g =>
      g.tableId === null &&
      g.status !== 'declinat' &&
    ...
    setGuests(prev => prev.map(g =>
      assignments[g.id] !== undefined
        ? { ...g, tableId: assignments[g.id] }
        : g
    ));
    ...
    if (limitReached) {
      showToast('⚠ Solutie partiala — limita atinsa, se foloseste cel mai bun rezultat gasit', 'yellow');
    }
  }, [showToast, saveAction]);
  ...
  const createTable = useCallback(() => {
    if (modal?.type !== 'bar' && !modal?.name?.trim()) { showToast('Introdu numele mesei!', 'red'); return; }
    saveAction();
    ...
    setTables(prev => [...prev, { id, name: modal.name.trim() || 'Bar', type: modal.type, seats: modal.seats || 0, x: cx, y: cy, rotation: 0 }]);
  ...
  const deleteTable = useCallback((tableId) => {
    ...
    setConfirmDialog({
      title: `Stergi "${t?.name}"?`,
      sub: t?.isRing ? 'Ring-ul dans va fi eliminat.' : t?.type === 'bar' ? 'Obiectul va fi eliminat.' : 'Invitatii revin in neatribuiti.',
      onOk: () => {
        saveAction();
        setGuests(prev => prev.map(g => g.tableId === tableId ? { ...g, tableId: null } : g));
        setTables(prev => prev.filter(x => x.id !== tableId));
        setEditPanel(null); setSelectedTableId(null);
  ...
  const rotateTable = useCallback((tableId, deg) => {
    setTables(prev => prev.map(t => t.id === tableId ? { ...t, rotation: ((t.rotation || 0) + deg) % 360 } : t));
  }, []);

  // ── SAVE EDIT ──
  const saveEdit = useCallback(() => {
    if (!editName.trim()) return;
    setTables(prev => prev.map(t => t.id === editPanel.tableId ? { ...t, name: editName.trim(), seats: editSeats } : t));
    setEditPanel(null);
