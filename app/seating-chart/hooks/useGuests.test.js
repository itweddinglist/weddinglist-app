...
describe('useGuests — return shape', () => {
  it('returnează toate cheile cerute', () => {
    ...
    const keys = [
      ...
      'progress', 'menuStats',
      'toasts', 'searchQuery', 'setSearchQuery',
...
describe('useGuests — hydration din storage', () => {
  it('guests și tables vin din storage dacă există date valide', () => {
    ...
    const guests = [{ id: 1, prenume: 'Test', nume: 'User', grup: 'Familie Mireasă', status: 'confirmat', meniu: 'Standard', tableId: null }];
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ guests, tables, nextId: 5, cam: DEFAULT_CAM }));
...
describe('useGuests — saveAction + undo', () => {
  it('saveAction salvează snapshot și undo îl restaurează', () => {
    ...
    act(() => { result.current.saveAction(); });
    act(() => {
      result.current.assignGuest(result.current.guests[0].id, result.current.tables.find(t => t.type !== 'bar' && !t.isRing)?.id);
    });
...
describe('useGuests — assignGuest', () => {
  it('guest neatribuit → atribuit la masă', () => {
    ...
    act(() => { result.current.assignGuest(guest.id, table.id); });
  ...
  it('masă plină → toast Masa este plină!', () => {
    ...
    if (remaining) {
      act(() => { result.current.assignGuest(remaining.id, table.id); });
  ...
  it('masă de tip bar → ignorat', () => {
    ...
    if (bar) {
      act(() => { result.current.assignGuest(guest.id, bar.id); });
...
describe('useGuests — unassignGuest', () => {
  it('guest atribuit → tableId devine null', () => {
    ...
    act(() => { result.current.assignGuest(guest.id, table.id); });
    act(() => { result.current.unassignGuest(guest.id); });
    expect(result.current.guests.find(g => g.id === guest.id).tableId).toBeNull();
  });
...
describe('useGuests — createTable', () => {
  it('masă nouă adăugată în tables', () => {
    ...
    act(() => { result.current.setModal({ type: 'round', name: 'Masa Test', seats: 8 }); });
  ...
  it('fără name → toast eroare', () => {
    const { result } = renderHook(() => useGuests(DEFAULT_CAM));
    act(() => { result.current.setModal({ type: 'round', name: '', seats: 8 }); });
    act(() => { result.current.createTable(); });
...
describe('useGuests — deleteTable', () => {
  it('setConfirmDialog apelat cu title corect', () => {
    ...
    act(() => { result.current.deleteTable(table.id); });
  ...
  it('după onOk: table ștearsă, guests unassigned', () => {
    ...
    act(() => { result.current.assignGuest(guest.id, table.id); });
    act(() => { result.current.deleteTable(table.id); });
    act(() => { result.current.confirmDialog.onOk(); });
...
describe('useGuests — rotateTable', () => {
  it('rotation incrementat cu deg % 360', () => {
    ...
    act(() => { result.current.rotateTable(table.id, 90); });
    expect(result.current.tables.find(t => t.id === table.id).rotation).toBe(90);

    act(() => { result.current.rotateTable(table.id, 300); });
...
describe('useGuests — stats', () => {
  ...
  it('progress === assignedCount/guests.length * 100', () => {
    ...
    act(() => { result.current.assignGuest(result.current.guests[0].id, table.id); });
...
describe('useGuests — filteredUnassigned', () => {
  it('searchQuery filtrează după prenume', () => {
    ...
    act(() => { result.current.setSearchQuery(firstName); });
    expect(result.current.filteredUnassigned.every(g =>
